...
autor = Autor.objects.create(nombre='Juan')
autor.save()
...
# ---- Test ---- Integrante: Eduardo Cercado y Cesar Miño

# 1. Crea un nuevo registro en el modelo Libro con los detalles especificados
...
# 2. Crea un autor llamado "Antoine" y relaciónalo con el libro "El Principito"
autor1 = Autor.objects.create(nombre="Antoine")
autor1.libros_autores.add(libro1)


# 3. Crea un libro con isbn "9876543210987", con los detalles especificados
libro2 = Libro.objects.create(
    isbn="9876543210987",
    titulo="La Biblia",
    paginas=300,
    fecha_publicacion="1973-01-01",
    imagen='http://imagen.com',
    estatus="B",
    categoria="Biblicas",
    desc_corta="Biblias",
...
# 4. Crea 4 autores y relaciónalos con los libros "El Principito" y "La Biblia" usando bulk_create
...
for autor in autores:
    autor.libros_autores.add(libro1, libro2)


# 5. Obtén todos los libros cuya categoría sea "Biblicas" y tengan más de 100 páginas
libros_biblicas = Libro.objects.filter(categoria="Biblicas", paginas__gt=100)


# 6. Dado el libro "La Biblia", mostrar todos sus autores
autores_biblia = libro2.libros_autores.all()


# 7. Obtén los libros publicados antes del año 2000 o que tengan más de 300 páginas
libros_antiguos_o_grandes = Libro.objects.filter(Q(fecha_publicacion__year__lt=2000) | Q(paginas__gt=300))


# 8. Actualiza el número de páginas de un libro específico, duplicándolo
libro1.paginas = F('paginas') * 2
libro1.save()


# 9. Incrementa el número de páginas en 10 para todos los libros que tengan más de 200 páginas
Libro.objects.filter(paginas__gt=200).update(paginas=F('paginas') + 10)


# 10. Cambia la categoría a "Educación" para todos los libros con estatus "C"
